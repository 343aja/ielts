class IELTSWritingTest {
  ...
  downloadAnswers() {
    ...
    content += `Time Remaining: ${Math.floor(this.currentTime / 60)}:${(
      this.currentTime % 60
    )
      .toString()
      .padStart(2, "0")}\n`;
    ...
    content += `\n`.repeat(2);
    content += `Word Count: ${
      task1Answer ? task1Answer.split(/\s+/).length : 0
    }\n`;
    content += `\n`.repeat(2);
    ...
    content += `\n`.repeat(2);
    content += `Word Count: ${
      task2Answer ? task2Answer.split(/\s+/).length : 0
    }\n`;
    ...
    setTimeout(() => {
      window.location.href = "./finish.html"; // o'zingizga kerakli sahifa nomini yozing
    }, 1000);
  }
  ...
  resetTest() {
    if (
      confirm(
        "Are you sure you want to reset the test? This will clear all your answers and reset the timer."
      )
    ) {
      // Clear answers
...
function initializeScrollControl() {
  ...
  // Question section scroll qilganda answer section harakatlanmasligi uchun
  questionSection.addEventListener("scroll", function (e) {
    // Bu yerda maxsus scroll logikasini qo'shish mumkin
    // Masalan, scroll pozitsiyasini saqlash yoki boshqa amallar
  });

  // Answer section fixed pozitsiyada turishi uchun
  // CSS da allaqachon position: fixed berilgan
}

// So'z sanashni yangilash funksiyasi
function updateWordCount() {
  ...
  // Rang o'zgarishi - minimum 150 so'z
  if (words >= 150) {
    wordCountElement.style.color = "#28a745"; // Yashil
  } else if (words >= 100) {
    wordCountElement.style.color = "#ffc107"; // Sariq
  } else {
    wordCountElement.style.color = "#dc3545"; // Qizil
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  updateWordCount(); // Boshlang'ich holatni o'rnatish

  // Textarea autofocus
  document.getElementById("answer-textarea").focus();
});

// Window resize da pozitsiyani qayta hisoblash
window.addEventListener("resize", function () {
  // Answer section pozitsiyasini qayta sozlash
  const answerSection = document.querySelector(".answer-section");
  // Agar kerak bo'lsa, bu yerda qo'shimcha sozlashlar qilish mumkin
});
