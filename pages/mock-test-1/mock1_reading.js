document.addEventListener('DOMContentLoaded', function () {
  ...
  function switchToPart(partNumber) {
    ...
    const selectedPassage = document.getElementById(`passage-text-${partNumber}`);
    if (selectedPassage) {
    ...
    const selectedQuestions = document.getElementById(`questions-${partNumber}`);
  ...
  function setupAutoSave() {
    ...
    document.querySelectorAll('.clickable-cell').forEach(cell => {
      cell.addEventListener('click', () => setTimeout(saveAnswersToStorage, 10));
    });
  ...
  function autoSubmitAnswers() {
    ...
    document.querySelectorAll('.tf-question, .multi-choice-question').forEach(q => {
      const qStart = parseInt(q.getAttribute('data-q-start'));
      const qEnd = parseInt(q.getAttribute('data-q-end'));
      for (let i = qStart; i <= qEnd; i++) {
        const checked = q.querySelector(`input[type="radio"][name="q${i}"]:checked`);
    ...
    document.querySelectorAll('.drop-zone[data-q]').forEach(zone => {
      ...
      answers.push({ question: q, answer: drag ? drag.getAttribute('data-value') : "" });
    });
    ...
    setTimeout(() => {
      window.location.href = './mock1_writing.html'; // o'zingizga kerakli sahifa nomini yozing
    }, 1000);
  }
  ...
  // Submit tugmasi
  document.getElementById('deliver-button').addEventListener('click', autoSubmitAnswers);
  ...
  function setupDragAndDrop() {
    ...
    dropZones.forEach(zone => {
      ...
      zone.addEventListener('drop', function(e) {
        ...
        const dragItem = document.querySelector(`.drag-item[data-value="${data}"]`);
  ...
  function setupMatchingTable() {
    document.querySelectorAll('.clickable-cell').forEach(cell => {
      cell.addEventListener('click', function() {
        ...
        document.querySelectorAll(`.clickable-cell[data-question="${question}"]`).forEach(c => {
